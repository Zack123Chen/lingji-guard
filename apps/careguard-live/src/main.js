...
import { generateReadableDataReport } from './report.js';
...
// ==========================================
        ...
        // ==========================================
        // 初始化中心点设在哈尔滨工业大学一校区
        const hitCoords = [45.7411, 126.6255];
        const map = L.map('map', {
            zoomControl: false,
        ...
        function buildTelemetryRecord(telemetry, displayState, isAlarm) {
            return {
                ...
                seq: telemetry.seq,
                ts: telemetry.ts,
                sampleReason: telemetry.sampleReason,
                heartContact: telemetry.heartContact,
                hrSampleType: telemetry.hrSampleType,
                ir: telemetry.ir,
                red: telemetry.red,
                ...
                coordinateSystem: telemetry.coordinateSystem,
                tf: telemetry.tf,
                tfReady: telemetry.tfReady,
                photo: telemetry.photo,
                photoFormat: telemetry.photoFormat,
                photoCount: telemetry.photoCount,
                noiseLevel: telemetry.noiseLevel,
                noiseEvent: telemetry.noiseEvent,
                audio: telemetry.audio,
                mode: telemetry.mode,
                lastCommand: telemetry.lastCommand,
                lastCommandResult: telemetry.lastCommandResult,
                lastCommandAt: telemetry.lastCommandAt
            };
        }
        ...
        function processIncomingTelemetry(payload) {
            try {
                ...
                const isAlarm = shouldTriggerHealthAlarm(telemetry);
                ...
                if (hasLocation) {
                    ...
                    previousPosition = hasGps ? currentPos : null;

                    if (hasGps) {
                        // 只有真实 GNSS 才记录轨迹；正心楼默认点只用于不中断地图显示。
                    ...
                    petMarker.setPopupContent(`
                        <div class="map-popup">
                            ...
                            心跳频率: ${hr} BPM<br>
                            核心体温: ${temp} °C<br>
                            定位: ${escapeHtml(formatGpsLabel(telemetry))}
                        </div>
                    `);
                }
        ...
        async function archiveCurrentSession() {
            const recentTelemetry = await getRecentTelemetry(200);
            const relevantTelemetry = recentTelemetry.filter(sample => sample.source === (isDemoMode ? "simulator" : "hardware"));
            if (routeCoordinates.length < 2 && relevantTelemetry.length === 0) {
                showToast("归档失败", "尚未产生有效健康监测记录，暂无法生成档案。", "error");
                return;
            }

            const newMission = createHealthArchiveMission({
                recentTelemetry: relevantTelemetry,
                routeCoordinates,
                cumulativeDistance,
                now: new Date()
            });
            ...
            try {
                ...
                logAction("数据", `档案成功写入：${newMission.type === "motion_route" ? "轨迹运动" : "健康监测"}，包含 ${newMission.telemetryPoints.length} 组真实体征样本。`);
            } catch (error) {
            ...
            archivedMissions.forEach((mission, index) => {
                tableBody.innerHTML += `
                    <tr onclick="loadArchivedMission(${index})">
                        ...
                        <td>${mission.path?.length >= 2 ? `${mission.distance.toFixed(1)} 米` : "无有效定位"}</td>
        ...
        async function exportStoredDataFile() {
            try {
                ...
                const report = generateReadableDataReport(data);
                const blob = new Blob([report], { type: "text/html;charset=utf-8" });
                const url = URL.createObjectURL(blob);
                const link = document.createElement("a");
                link.href = url;
                link.download = `careguard-report-${new Date().toISOString().slice(0, 10)}.html`;
                document.body.appendChild(link);
                link.click();
                link.remove();
                URL.revokeObjectURL(url);
                showToast("报告导出", "已生成可直接打开和打印的健康监测报告。", "success");
            } catch (error) {
        ...
        function loadArchivedMission(index) {
            ...
            const points = Array.isArray(mission.path) ? mission.path.map(p => new L.LatLng(p[0], p[1])) : [];
            if (points.length >= 2) {
            ...
            const replay = buildMissionReplaySeries(mission);
            telemetryChart.data.labels = replay.labels;
            telemetryChart.data.datasets[0].data = replay.hr;
            telemetryChart.data.datasets[1].data = replay.temp;
            telemetryChart.update();
            ...
            document.getElementById('kpiDistance').innerText = points.length >= 2 ? `${mission.distance} 米` : "无有效定位";

            logAction("数据", `已加载并重绘体征档案: '${mission.name}'，真实样本 ${replay.labels.length} 组，轨迹点 ${points.length} 个。`);
