...
void printHelp() {
  ...
  Serial.println("  camdump  - disabled; image upload/dump is not used");
  Serial.println("  camjpeg  - disabled; JPEG capture is not used");
  Serial.println("  btn      - print camera shutter button pin level");
  Serial.println("  tf       - print TF card telemetry status");
  Serial.println("  tft      - show TFT color self-test, then force dashboard redraw");
  Serial.println("  snap     - show one local grayscale preview, no file saved");
