...
String buildJson(const SensorData& data,
  ...
  appendUIntField(out, first, "photoCount", static_cast<unsigned long>(storage.photo_count));
  appendStringField(out, first, "photo", storage.last_photo_path.length() ? storage.last_photo_path : storage.last_status);
  appendStringField(out, first, "photoFormat", storage.last_photo_format);
