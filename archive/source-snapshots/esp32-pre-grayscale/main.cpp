...
namespace {
constexpr const char* kDiagDir = "/camera_jpeg_diag";
constexpr uint8_t kDropFrames = 3;
constexpr uint8_t kSavedFrames = 7;

camera_config_t makeCameraDiagConfig() {
...
bool mountTfCard() {
  ...
  if (!SD_MMC.setPins(Pins::Storage::CLK, Pins::Storage::CMD, Pins::Storage::DATA0)) {
    Serial.println("[CAMJPEG] TF setPins failed");
    return false;
  }
  if (!SD_MMC.begin("/sdcard", true, false, 4000, 5)) {
    Serial.println("[CAMJPEG] TF mount failed");
    return false;
  }
  SD_MMC.mkdir(kDiagDir);
  Serial.printf("[CAMJPEG] TF mounted size=%lluMB used=%lluMB\n",
                SD_MMC.cardSize() / (1024ULL * 1024ULL),
                SD_MMC.usedBytes() / (1024ULL * 1024ULL));
  return true;
}

bool saveFb(camera_fb_t* fb, const char* label, uint8_t index) {
  char path[96] = {};
  snprintf(path, sizeof(path), "%s/%s_%02u_%lu.jpg", kDiagDir, label, index, static_cast<unsigned long>(millis()));
  File file = SD_MMC.open(path, FILE_WRITE);
  if (!file) {
    Serial.printf("[CAMJPEG] SAVE_FAIL open path=%s\n", path);
    return false;
  }
  const size_t written = file.write(fb->buf, fb->len);
  file.close();
  const bool ok = written == fb->len;
  Serial.printf("[CAMJPEG] SAVE_%s path=%s w=%u h=%u len=%u written=%u format=%d\n",
                ok ? "OK" : "SHORT",
                path,
...
void applyCameraDiagSettings(bool colorbar) {
  camera_sensor_t* sensor = esp_camera_sensor_get();
  if (sensor == nullptr) {
    Serial.println("[CAMJPEG] sensor missing after init");
    return;
  }
  sensor->set_pixformat(sensor, PIXFORMAT_JPEG);
  sensor->set_framesize(sensor, FRAMESIZE_QQVGA);
  sensor->set_quality(sensor, 10);
  sensor->set_colorbar(sensor, colorbar ? 1 : 0);
  sensor->set_whitebal(sensor, 1);
  sensor->set_awb_gain(sensor, 1);
  sensor->set_exposure_ctrl(sensor, 1);
  sensor->set_gain_ctrl(sensor, 1);
  Serial.printf("[CAMJPEG] OV2640 settings colorbar=%s format=JPEG frame=QQVGA fb_count=1 grab=WHEN_EMPTY\n",
                colorbar ? "on" : "off");
}

void captureAndMaybeSave(uint8_t frame_index, bool save, const char* label) {
  camera_fb_t* fb = esp_camera_fb_get();
  if (fb == nullptr) {
    Serial.printf("[CAMJPEG] FRAME_%u FAIL fb=null\n", static_cast<unsigned>(frame_index));
    return;
  }

  Serial.printf("[CAMJPEG] FRAME_%u w=%u h=%u len=%u format=%d save=%s\n",
                static_cast<unsigned>(frame_index),
                static_cast<unsigned>(fb->width),
                static_cast<unsigned>(fb->height),
                static_cast<unsigned>(fb->len),
                ...
                save ? "yes" : "no");
  if (save) {
    saveFb(fb, label, frame_index);
  }
  esp_camera_fb_return(fb);
  delay(120);
}

void runCameraJpegDiagnostic() {
  Serial.println("[CAMJPEG] Camera JPEG diagnostic firmware");
  Serial.println("[CAMJPEG] TFT preview, RGB565, BMP conversion, and auto snapshot are disabled.");
  Serial.printf("[CAMJPEG] psramFound=%s total=%u free=%u\n",
                psramFound() ? "true" : "false",
                static_cast<unsigned>(ESP.getPsramSize()),
                static_cast<unsigned>(ESP.getFreePsram()));
  Serial.printf("[CAMJPEG] heap internal free=%u largest=%u psram free=%u largest=%u\n",
  ...
  Serial.printf("[CAMJPEG] pins D0-D7=%d,%d,%d,%d,%d,%d,%d,%d PCLK=%d VSYNC=%d HREF=%d XCLK=%d SIOD=%d SIOC=%d\n",
                Pins::Camera::D0,
                Pins::Camera::D1,
                Pins::Camera::D2,
                Pins::Camera::D3,
  ...
  const camera_config_t config = makeCameraDiagConfig();
  const esp_err_t err = esp_camera_init(&config);
  if (err != ESP_OK) {
    Serial.printf("[CAMJPEG] camera init failed: 0x%x\n", err);
    return;
  }
  applyCameraDiagSettings(false);
  delay(300);

  for (uint8_t i = 0; i < kDropFrames + kSavedFrames; ++i) {
    captureAndMaybeSave(i, i >= kDropFrames, "scene");
  }

  applyCameraDiagSettings(true);
  delay(500);
  captureAndMaybeSave(0, true, "colorbar");
  Serial.println("[CAMJPEG] diagnostic complete; inspect /camera_jpeg_diag/*.jpg on TF.");
}
}  // namespace
...
void setup() {
  ...
  delay(15000);
  Serial.println();
  runCameraJpegDiagnostic();
}

void loop() {
  delay(5000);
  Serial.println("[CAMJPEG] idle");
}
...
constexpr uint32_t kSnapshotDurationMs = 2000;
...
constexpr bool kCameraBootSelfTestEnabled = true;
...
void drawCameraStatusForPhoto(const PhotoStatus& photo) {
  if (photo.saved) {
    DisplayManager::drawCameraStatus("照片已保存", photo.path.c_str(), kUiGreen);
  } else if (photo.status == "JPEG check failed") {
    DisplayManager::drawCameraStatus("照片校验失败", photo.status.c_str(), kUiOrange);
  } else {
    DisplayManager::drawCameraStatus("相机暂不可用", photo.status.c_str(), kUiRed);
  }
  g_camera_preview_latched = false;
  g_preview_until_ms = millis() + kSnapshotDurationMs;
}

bool captureAndSaveJpegSnapshot(const char* reason) {
  const bool is_auto = isAutoCameraReason(reason);
  if (!ensureCameraReadyForCapture(is_auto)) {
    if (!is_auto) {
      DisplayManager::drawCameraStatus("相机暂不可用", "初始化失败", kUiRed);
  ...
  PhotoStatus photo;
  DisplayManager::drawCameraStatus("正在拍照", "JPEG QQVGA 存档", kUiCyan);
  const bool photo_saved = StorageManager::captureAndSaveJpeg(reason, photo);
  Serial.printf("[CAM] trigger=%s result=%s path=%s format=%s len=%lu written=%lu status=%s\n",
                reason ? reason : "Snapshot",
                photo_saved ? "OK" : "FAIL",
                photo.path.c_str(),
                photo.format.c_str(),
                static_cast<unsigned long>(photo.len),
                static_cast<unsigned long>(photo.written),
                photo.status.c_str());
  drawCameraStatusForPhoto(photo);
  if (photo_saved) {
    g_camera_consecutive_failures = 0;
  } else if (g_camera_consecutive_failures < 255) {
    ++g_camera_consecutive_failures;
    if (g_camera_consecutive_failures >= 3) {
      CameraManager::markLimited();
      Serial.println("[CAM] Camera Limited after 3 consecutive JPEG/TF failures; automatic retry remains disabled.");
    }
  }
  return photo_saved;
}
...
bool triggerCameraSnapshot(const char* source) {
  g_camera_preview_latched = false;
  Serial.printf("[%s] Camera JPEG snapshot requested.\n", source ? source : "Snapshot");
  return captureAndSaveJpegSnapshot(source);
}
...
void handleAutoCameraSnapshot(uint32_t now) {
  ...
  captureAndSaveJpegSnapshot("Auto");
}
...
void initCameraShutterButton() {
  ...
  Serial.printf("Camera shutter button: GPIO%d active-low raw=%s. Short press captures one frame.\n",
...
void handleRemoteCommand(const RemoteCommandEvent& event) {
  ...
  switch (event.command) {
    ...
    case RemoteCommand::Capture:
      {
        const bool ok = triggerCameraSnapshot("MQTT");
        recordCommandResult(event.code, ok ? "ok" : "camera_limited");
      }
      AudioManager::startAckBeep();
      return;
...
void handleCameraDumpCommand() {
  Serial.println("CAMFRAME_DISABLED RGB565 preview/dump is disabled; use camjpeg or snap.");
}

void handleCameraJpegCommand() {
  CameraFrame frame;
  if (!CameraManager::captureJpegFrame(frame)) {
    Serial.println("CAMJPEG_ERROR capture_failed");
    return;
  }

  Serial.printf("CAMJPEG_BEGIN %u %u %u %d\n",
                static_cast<unsigned>(frame.width),
                static_cast<unsigned>(frame.height),
                static_cast<unsigned>(frame.len),
                frame.format);
...
void setup() {
  ...
  if (kCameraBootSelfTestEnabled) {
    PhotoStatus boot_photo;
    const bool boot_ok = StorageManager::captureAndSaveJpeg("BootSelfTest", boot_photo);
    Serial.printf("[CAM] boot self-test result=%s path=%s format=%s len=%lu written=%lu status=%s\n",
                  boot_ok ? "OK" : "FAIL",
                  boot_photo.path.c_str(),
                  boot_photo.format.c_str(),
                  static_cast<unsigned long>(boot_photo.len),
                  static_cast<unsigned long>(boot_photo.written),
                  boot_photo.status.c_str());
  }
  SensorManager::read();
  drawDashboard();
