...
String storageCn(const ModuleState& storage_state, const StorageData& storage) {
  if (storage_state.ready) {
    if (storage.photo_count > 0) {
      return "照片 " + String(static_cast<unsigned long>(storage.photo_count));
    }
    return "记录中";
  }
...
void drawCameraFrame(const CameraFrame& frame) {
  (void)frame;
  drawCameraStatus("低清存档", "不显示预览", ST77XX_CYAN);
}

void drawCameraCaptureFailed() {
  drawCameraStatus("相机暂不可用", "请检查排线与TF卡", ST77XX_RED);
}
...
void drawCameraStatus(const char* title, const char* subtitle, uint16_t color) {
  ...
  drawCenterCn(118, subtitle && subtitle[0] ? subtitle : "低清 JPEG 存档", ST77XX_WHITE);
  g_tft.drawRect(24, 150, 192, 34, color);
  drawCenterCn(174, "低清 JPEG 存档（实验）", color);
  g_tft.setTextSize(1);
}
